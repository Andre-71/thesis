...
class MergerSorterStorage(Consumer, ConsumerStorage):
    def __init__(self):
    ...
    def process(self, data):
        if self.message.topic == 'input':
            ...
            data = compress_encoding(compressed_image_array, ENCODING)
        self.message.value = ''
        return data
...
class MergerSorterProducer(Producer, CsvLogging):
    def __init__(self, consumer_storage, loop=None):
        ...
        self.avg_delay = 0
        self.n_avg_delay = 0
        self.thresh = int(os.getenv('WAIT_THRESH', 5000))
...
async def main():
    consumer = MergerSorterStorage()
    producer = MergerSorterProducer(consumer)
