...
class UAVFrameProducerStorage(UAVFrameConsumer, ConsumerStorage):
  ...
  def process(self, data):
    ...
    reduced_image_for_show = image.resize((640, 480), resample=Image.LANCZOS)
    reduced_image_for_show_array = np.array(reduced_image_for_show)
    data_for_compression = super().process(reduced_image_for_show_array)
    image_for_compression = Image.fromarray(data_for_compression)
    reduced_image = image_for_compression.resize(self.frame_size, resample=Image.LANCZOS)
    data = np.array(reduced_image)
    return data
